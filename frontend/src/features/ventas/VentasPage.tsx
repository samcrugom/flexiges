...
export default function VentasPage() {
	...
	const verPdf = async (id: number) => {
		try {
			// Usa el mismo `get` que usan el resto de llamadas
			//const response = await get(`/ventas/${id}/pdf`, { responseType: 'blob' })
	...
	const cols = [
		{ key: 'numero', label: 'Númeroaaaa', render: (r: VentaDocumento) => <span className="font-mono text-[12px]">{r.numero}</span> },
		...
		{ key: 'estado', label: 'Estado', render: (r: VentaDocumento) => <EstadoBadge estado={r.estado} /> },
		{
			key: '_', label: '', render: (r: VentaDocumento) => (
				<div className="flex gap-1">
					{r.tipo === 'factura' && <button onClick={e => { e.stopPropagation(); verPdf(r.id) }} className="p-1.5 rounded-[6px] border border-[#E5E3DC] bg-white hover:bg-[#F0EFE9] text-[#6B6860]" title="Ver PDF"><Eye size={13} /></button>}
